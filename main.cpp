...
//Output:
//	None
void initField(int num_of_mine) { // Initializes all cell values in the minesweeper game to be unknown
...
bool isMine(int i, int j) {
    int returnvalue = getCell(i,j);
    if (returnvalue==FLAGGED_MINE || returnvalue==UNFLAGGED_MINE) {
        return true;
    }
    else{
        return false;
...
void reveal(int i,int j) {
    int counter = 0;
    if ((i + 1) != 8 && isMine(i+1,j) ){ // Checks 1 row up for mine
        counter +=1;
    }
    if ((i-1)!=-1 && isMine(i - 1,j) ) { // Checks 1 row down for mine
        counter += 1;
    }
    if ((j+1)!=5 && isMine(i,j+1) ) { // Checks 1 column to the right for mine
        counter += 1;
    }
    if ((j-1)!=-1 && isMine(i,j-1) ) { // Checks 1 column to the left for mine
        counter += 1;
    }
    if ((j+1)!=5 && (i+1)!=8 && isMine(i+1,j+1)) { // Checks diagonally to the right down
        counter += 1;
    }
    if ((j-1)!=-1 && (i+1)!=8 && isMine(i+1,j-1) ) { // Checks diagonally to the left down
        counter += 1;
    }
    if ((j-1)!=-1 && (i-1)!=-1 && isMine(i-1,j-1) ) { // Checks diagonally to the left up
        counter += 1;
    }
    if ((j+1)!=5 && (i-1)!=-1 && isMine(i-1,j+1) ) { // Checks diagonally to the right up
        counter += 1;
    }
...
//	None
void executeCmd(string cmd) {
    string lettercommand = cmd.substr(0, 1);
    string sstring1 = cmd.substr(1,1);
    string sstring2 = cmd.substr(2,1);
    int row = stoi(sstring1);
    int column = stoi(sstring2);
    if (lettercommand == "r"){ // Reveal command
        if ((getCell(row,column))==UNFLAGGED_MINE || (getCell(row,column))==FLAGGED_MINE){
            explode = true;
        }
        else {
            reveal(row, column);
    ...
    else if (lettercommand == "f"){ // Flag command
        getCell(row,column);
        if (getCell(row,column)!=UNFLAGGED_MINE){
            ...
            int mineValue = getCell(row, column);
            num_correct_flagged_mine+=1;
        }


    }
    else if (lettercommand == "u"){ // Undo command
